export var MazeManager = {
  ...
  opposite: function (direction) {
    ...
    switch (direction) {
      case MazeManager.DIRECTION.TOP:
        result = MazeManager.DIRECTION.BOTTOM;
        break;
      case MazeManager.DIRECTION.RIGHT:
        result = MazeManager.DIRECTION.LEFT;
        break;
      case MazeManager.DIRECTION.BOTTOM:
        result = MazeManager.DIRECTION.TOP;
        break;
      case MazeManager.DIRECTION.LEFT:
        result = MazeManager.DIRECTION.RIGHT;
        break;
      default:
        result = MazeManager.DIRECTION.RIGHT;
        break;
  ...
  carve: function (x, y) {
    ...
    directions.forEach(function (direction) {
      if (direction === 0 && y - 1 >= 0 && MazeManager.grid[y - 1][x] === 0) {
        ...
        direction === 1 &&
        x + 1 < MazeManager.grid[0].length &&
        MazeManager.grid[y][x + 1] === 0
      ) {
        // Right.
        MazeManager.path(x, y, x + 1, y, MazeManager.DIRECTION.RIGHT);
      } else if (
        direction === 2 &&
        y + 1 < MazeManager.grid.length &&
        MazeManager.grid[y + 1][x] === 0
      ) {
        // Bottom.
